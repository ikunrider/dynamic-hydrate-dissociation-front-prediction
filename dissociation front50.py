...
X = X.values
X.shape
y = pd.read_csv('E:\\进展\\滑坡预测\\output50.csv',header=None)
y = y.values
y.shape
seed = 7
test_size = 0.1
...
df.to_csv('D:\\xgboost_evaluation_data_50.csv', index=False)
...
t.to_csv('D:\\classification_report_50.csv')
...
df2 = pd.DataFrame(yt)
# Save the DataFrame to a CSV file
df1.to_csv('D:\\y_pred_50.csv', index=False)
df1.to_csv('D:\\y_test_50.csv', index=False)
...
for i in range(100):
    ...
    plt.ylabel('50% Dissociation front(m)',font3)
...
for i in l:
    ...
    plt.ylabel('50% Dissociation front(m)',font1)
    plt.legend(loc="lower right",prop=font2)
    plt.tight_layout()
    plt.savefig("D:\\50%d.tiff"%i)
    plt.show()
    plt.close()
...
# Save the DataFrame to a CSV file
df.to_csv('D:\\top_feature_importance_50.csv', index=False)
...
f_m = ['Time','Residual water saturation','Subloading ratio evolution','Stiffness enhancement by hydrate','Initial porosity',
       ...
       'Ideal gas saturation','Van genuchten parameter c','Hydrate number']
# 仅保留前10个特征的重要度
top_importance = {feature: importance[feature] for feature in top_features}
...
plt.tight_layout()
plt.savefig("D:\\50_importance.tiff")
plt.show()
...
plt.tight_layout()
plt.savefig("D:\\50_importance_.tiff")
plt.show()
