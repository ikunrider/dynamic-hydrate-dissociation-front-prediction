...
X = X.values
X.shape
y = pd.read_csv('E:\\进展\\滑坡预测\\output80.csv',header=None)
y = y.values
y.shape
seed = 7
test_size = 0.1
...
df.to_csv('D:\\xgboost_evaluation_data_80.csv', index=False)
...
plt.xlim([0.0, 600])
plt.ylim([0.0, 3])
plt.yticks(np.arange(0.5,3.5,0.5),fontproperties='Times New Roman', size=25)#设置大小及加粗
plt.xticks(np.arange(0,700,100),fontproperties='Times New Roman', size=25)
ax.legend(prop=font2)
plt.ylabel('Log Loss',font1)
plt.xlabel("n_trees",font1)
plt.tight_layout()
#plt.title('XGBoost Log Loss',font1)
plt.savefig("D:\\loss80.tiff")
plt.show()
plt.close()
...
plt.xlim([0.0, 600])
plt.ylim([0.0, 0.8])
plt.yticks(np.arange(0.1,0.9,0.1),fontproperties='Times New Roman', size=25)#设置大小及加粗
plt.xticks(np.arange(0,700,100),fontproperties='Times New Roman', size=25)
ax.legend(prop=font2)
...
t.to_csv('D:\\classification_report_80.csv')
...
df2 = pd.DataFrame(yt)
# Save the DataFrame to a CSV file
df1.to_csv('D:\\y_pred_80.csv', index=False)
df1.to_csv('D:\\y_test_80.csv', index=False)
...
for i in range(100):
    ...
    plt.ylabel('80% Dissociation front(m)',font3)
...
for i in l:
    ...
    plt.xlim([0, 31])
    plt.ylim([0, yt[i,59]+1])
    plt.yticks(np.arange(0,yt[i,59]+1,5),fontproperties='Times New Roman', size=50)#设置大小及加粗
    plt.xticks([1,5,10,15,20,25,30],fontproperties='Times New Roman', size=50)
    plt.tick_params(labelsize=30)
    plt.xlabel('Elapsed time(days)',font1)
    plt.ylabel('80% Dissociation front(m)',font1)
    plt.legend(loc="lower right",prop=font2)
    plt.tight_layout()
    plt.savefig("D:\\80%d.tiff"%i)
    plt.show()
    plt.close()
...
# Save the DataFrame to a CSV file
df.to_csv('D:\\top_feature_importance_80.csv', index=False)
...
# 将特征重要度排序并选择前10个
top_features = sorted(importance, key=importance.get, reverse=True)[:10]
f_m = ['Time','Residual water saturation','Subloading ratio evolution','Stiffness enhancement by hydrate','Initial porosity',
       'Stress ratio at the critical state','Well pressure',
       'Ideal gas saturation','Van genuchten parameter c','Entry capillary pressure']
# 仅保留前10个特征的重要度
top_importance = {feature: importance[feature] for feature in top_features}
...
plt.tick_params(axis='x',direction='in',top=True,right=True)
plt.tick_params(axis='y',direction='in',top=True,right=True)
plt.barh(f_m,top_importance.values(),color='black')
plt.xlabel('Features',font1)
plt.ylabel('Importance',font1)
plt.title('Top 10 Feature Importance',font1)
plt.yticks(fontproperties='Times New Roman', size=25)
plt.xticks(rotation=0,fontproperties='Times New Roman', size=25)
plt.tight_layout()
plt.savefig("D:\\80_importance.tiff")
plt.show()
...
plt.tight_layout()
plt.savefig("D:\\80_importance_.tiff")
plt.show()
